  ...
  return (
    <div className="nb-wrap" ref={panelRef}>
      ...
      >
        ...
        {loading && (
          <span className="nb-bar__loading">Loading news…</span>
        )}

        <span className={`nb-bar__chevron ${open ? "nb-bar__chevron--up" : ""}`}>
          ▾
        </span>
      </button>
        ...
        <div className="nb-panel">
          <div className="nb-panel__list">
            {articles.map((a, i) => (
              ...
              >
                <span className="nb-article__num">{String(i + 1).padStart(2, "0")}</span>
                <div className="nb-article__body">
                  <span className="nb-article__title">{a.title}</span>
                  {a.desc && <span className="nb-article__desc">{a.desc}…</span>}
                </div>
                <div className="nb-article__meta">
                  {a.pubDate && <span className="nb-article__age">{timeAgo(a.pubDate)}</span>}
...
function NextRaceCard() {
  ...
  const [countdown, setCountdown] = useState({ days: "--", hrs: "--", min: "--", sec: "--" });
  ...
  useEffect(() => {
    async function fetchNext() {
      try {
        ...
        const now   = new Date();
        const next  = races.find((r) => new Date(`${r.date}T${r.time || "00:00:00"}`) > now);
        if (next) setRace(next);
      } catch (e) { console.error("NextRaceCard:", e); }
    }
    fetchNext();
  }, []);
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      if (diff <= 0) { setCountdown({ days:"00", hrs:"00", min:"00", sec:"00" }); return; }
  ...
  const timeStr  = race.time
    ? raceDate.toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit", timeZoneName: "short" })
    : null;
  ...
  return (
    <div className="nrc-card">
      ...
      <div className="nrc-countdown">
        {[[countdown.days,"Days"],[countdown.hrs,"Hrs"],[countdown.min,"Min"],[countdown.sec,"Sec"]].map(([val,lbl]) => (
      ...
      <a href="#schedule" className="nrc-cta">View Schedule →</a>
    </div>
  );
}
    ...
    <div>
      ...
      <section id="home" className="hero">
        <div className="slideshow">
          <div className="slide" style={{ backgroundImage: "url('/img5.png')" }} />
          <div className="slide" style={{ backgroundImage: "url('/img2.png')" }} />
          <div className="slide" style={{ backgroundImage: "url('/img1.png')" }} />
          <div className="slide" style={{ backgroundImage: "url('/img3.png')" }} />
          <div className="slide" style={{ backgroundImage: "url('/img4.png')" }} />
          <div className="slide" style={{ backgroundImage: "url('/img6.png')" }} />
        </div>

        <div className="hero-inner container">
          <div className="hero-eyebrow">Formula 1 · 2026 World Championship</div>
          <h1>Experience the<br />Thrill of<br /><span className="hero-accent">Formula 1</span></h1>
          <p>
            Live speed. Live strategy. Live drama.<br />
      ...
      {/* ── LATEST RACE STATS ────────────────────────────────── */}
      <section id="stats" className="home-section home-section--narrow container">
